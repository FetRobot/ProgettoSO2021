...
#include "disastrOS_structures.h" //lf header with new queue' structures
...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){
  ...
  syscall_vector[DSOS_CALL_WRITE_QUEUE] 	= writeQueue; //lf new system call
  syscall_numarg[DSOS_CALL_WRITE_QUEUE]		= 3;		 //lf new system call

  syscall_vector[DSOS_CALL_READ_QUEUE] 	= readQueue;	   //lf new system call
  syscall_numarg[DSOS_CALL_READ_QUEUE]		= 3;		 //lf new system call
...
int disastrOS_write_queue (queue* queue, char* s, int size) {		//lf new system call
  return disastrOS_syscall(DSOS_CALL_WRITE_QUEUE, queue, s, size);			//lf new system call
}

int disastrOS_read_queue (queue* queue, char* s, int size) {		//lf new system call
  return disastrOS_syscall(DSOS_CALL_READ_QUEUE, queue, s, size);			//lf new system call
}
